...
const Dialogs = (props) => {
  ...
  let DialogElements = state.dialogData.map((p) => (
    <DialogItem name={p.name} id={p.id} />
  ));

  let MessageElements = state.messageData.map((item) => (
    <Message
      deletMessage={props.deletMessage}
  ...
  return (
    <div className={x.dialogs}>
      <div className={x.dialogsItems}>{DialogElements}</div>
